...
function c120000051.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(aux.FilterFaceupFunction(Card.IsAttribute,ATTRIBUTE_LIGHT),tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectTarget(tp,aux.FilterFaceupFunction(Card.IsAttribute,ATTRIBUTE_LIGHT),tp,LOCATION_ONFIELD,0,1,1,nil)
end
