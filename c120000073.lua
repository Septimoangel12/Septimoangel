...
function c120000073.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(120000073,0))
...
function c120000073.condition(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(Card.IsType,e:GetHandlerPlayer(),LOCATION_ONFIELD,0,2,nil,TYPE_MONSTER) or Duel.GetCurrentChain()~=0 then return end
...
function c120000073.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return eg:IsExists(c120000073.repfilter,1,nil,tp) and Duel.GetCurrentChain()==0 end
	if Duel.IsExistingMatchingCard(Card.IsType,e:GetHandlerPlayer(),LOCATION_ONFIELD,0,1,ec,TYPE_MONSTER) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESREPLACE)
		local g=Duel.SelectReleaseGroup(tp,Card.IsType,1,1,ec,TYPE_MONSTER)
