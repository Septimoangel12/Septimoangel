...
function c120000085.initial_effect(c)
	...
	c:RegisterEffect(e1)
	aux.GlobalCheck(c120000085,function()
		c120000085[0]=0
		c120000085[1]=0
...
function c120000085.checkfilter(c,tp)
	...
	return ec and c:IsType(TYPE_SYNCHRO) and c:GetEquipTarget()~=nil or c:IsReason(REASON_LOST_TARGET) 
		and ( ec:IsSetCard(0x3013) or ec:IsSetCard(0x562) or ec:IsCode(63468625) ) and c:GetControler(tp)
end
...
function c120000085.condition(e,tp,eg,ep,ev,re,r,rp)
	return c120000085[tp] and  Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsInfinity),e:GetHandlerPlayer(),LOCATION_ONFIELD,0,1,e:GetHandler())
end
function c120000085.tgfilter(c,tid)
	local ec=c:GetPreviousEquipTarget()
	return c:IsType(TYPE_SYNCHRO) and c:GetTurnID()==tid and c:IsPreviousLocation(LOCATION_SZONE) and ec and ( ec:IsSetCard(0x3013) or ec:IsSetCard(0x562) or ec:IsCode(63468625) )
...
function c120000085.eqfilter(c)
	return c:IsFaceup() and ( c:IsSetCard(0x3013) or ec:IsSetCard(0x562) or c:IsCode(63468625) ) 
end
function c120000085.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,c120000085.eqfilter,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,1,nil):GetFirst()
	local eq=g:GetFirst()
	while eq do
	if not Duel.Equip(tp,eq,tg,false) then return end
		...
		eq:RegisterFlagEffect(120000085,RESET_EVENT+RESETS_STANDARD,0,0)
