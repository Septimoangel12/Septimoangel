...
function c511004004.cfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsFaceup() and c:IsRace(RACE_SPELLCASTER)
end
function c511004004.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c511004004.cfilter,tp,LOCATION_ONFIELD,0,1,nil)
end
function c511004004.filter(c,e,tp)
	return c:IsRace(RACE_SPELLCASTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c511004004.rfilter(c,fid)
	return c:IsType(TYPE_MONSTER) and c:GetFieldID()~=fid
end
function c511004004.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local g=Duel.GetMatchingGroup(c511004004.cfilter,tp,LOCATION_ONFIELD,0,nil)
	...
	local g1=Duel.GetMatchingGroup(c511004004.cfilter,tp,LOCATION_ONFIELD,0,nil)
	...
	elseif ct==2 then
		...
		g1=Duel.GetMatchingGroup(c511004004.cfilter,tp,LOCATION_ONFIELD,0,g:GetFirst())
...
function c511004004.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()==2 then
		...
		if Duel.IsExistingMatchingCard(c511004004.cfilter,tp,LOCATION_ONFIELD,0,2,nil) 
			and Duel.IsExistingMatchingCard(c511004004.dfilter,tp,0,LOCATION_ONFIELD,1,nil) then
			local sg=Duel.SelectMatchingCard(tp,c511004004.cfilter,tp,LOCATION_ONFIELD,0,2,2,nil,e,tp)
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
			local dg=Duel.SelectMatchingCard(tp,c511004004.dfilter,tp,0,LOCATION_ONFIELD,1,1,nil):GetFirst()
			...
			Duel.BreakEffect()
			if dg and not (dg:IsHasEffect(EFFECT_INDESTRUCTABLE_BATTLE) or dg:IsHasEffect(EFFECT_INDESTRUCTABLE_COUNT)) and Duel.Destroy(dg,REASON_BATTLE) then
				Duel.SendtoGrave(dg,REASON_BATTLE)
				dg:SetStatus(STATUS_BATTLE_DESTROYED,true)
				Duel.RaiseSingleEvent(dg,EVENT_BATTLE_DESTROYED,e,REASON_BATTLE,tp,tp,0)
		end
			Duel.RaiseEvent(dg,EVENT_BATTLE_DESTROYED,e,REASON_BATTLE,tp,tp,0) end
	end
end
