...
function c511004004.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()==2 then
		...
		and Duel.IsExistingMatchingCard(Card.IsDestructable,tp,0,LOCATION_MZONE,1,nil) then
			...
			local dg=Duel.SelectMatchingCard(tp,c511004004.dfilter,tp,0,LOCATION_MZONE,1,1,nil)
			...
			local tg=dg:Filter(Card.IsLocation,nil,LOCATION_MZONE)
			Duel.SendtoGrave(tg,REASON_BATTLE)
		end
